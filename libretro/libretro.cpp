...
enum lightgun_mode { SETTING_GUN_INPUT_LIGHTGUN, SETTING_GUN_INPUT_POINTER };
...
void retro_set_environment(retro_environment_t cb) {
  ...
  static const struct retro_subsystem_rom_info multicart_roms[] = {
      {"Cart A", "smc|sfc|swc|fig|bs|st", false, false, false, multi_a_memory,
       1},
      {"Cart B", "smc|sfc|swc|fig|bs|st", false, false, false, multi_b_memory,
       1},
  };

  static const struct retro_subsystem_info subsystems[] = {
      {"Multi-Cart Link", "multicart_addon", multicart_roms, 2,
  ...
  static const struct retro_controller_info ports[] = {
      {port_1, 4},     {port_2, 7},     {port_3, 3},
      {port_extra, 2}, {port_extra, 2}, {port_extra, 2},
      {port_extra, 2}, {port_extra, 2}, {},
  };
...
char *get_cursor_color(const char *name) {
  static char color_names[][32] = {
      "Black",    "Black",     "Black (blend)",    "tBlack",
      "25% Grey", "25Grey",    "25% Grey (blend)", "t25Grey",
      "50% Grey", "50Grey",    "50% Grey (blend)", "t50Grey",
      "75% Grey", "75Grey",    "75% Grey (blend)", "t75Grey",
      "White",    "White",     "White (blend)",    "tWhite",
      "Red",      "Red",       "Red (blend)",      "tRed",
      "Orange",   "Orange",    "Orange (blend)",   "tOrange",
      "Yellow",   "Yellow",    "Yellow (blend)",   "tYellow",
      "Green",    "Green",     "Green (blend)",    "tGreen",
      "Cyan",     "Cyan",      "Cyan (blend)",     "tCyan",
      "Sky",      "Sky",       "Sky (blend)",      "tSky",
      "Blue",     "Blue",      "Blue (blend)",     "tBlue",
      "Violet",   "Violet",    "Violet (blend)",   "tViolet",
      "Pink",     "MagicPink", "Pink (blend)",     "tMagicPink",
      "Purple",   "Purple",    "Purple (blend)",   "tPurple",
      "\0",       "\0"};
...
static void update_variables(void) {
  ...
  // for some reason, Transparency seems to control both the fixed color and the
  // windowing registers?
  ...
  var.value = NULL;
  Settings.Transparency = !(environ_cb(RETRO_ENVIRONMENT_GET_VARIABLE, &var) &&
                            !strcmp("disabled", var.value));
  ...
  if (environ_cb(RETRO_ENVIRONMENT_GET_VARIABLE, &var)) {
    ...
    if (color[0] == 't')
      S9xSetControllerCrosshair(X_SUPERSCOPE, -1, get_cursor_color(var.value),
                                "tBlack");
    else
      S9xSetControllerCrosshair(X_SUPERSCOPE, -1, get_cursor_color(var.value),
                                "Black");
  ...
  if (environ_cb(RETRO_ENVIRONMENT_GET_VARIABLE, &var)) {
    ...
    if (color[0] == 't')
      S9xSetControllerCrosshair(X_JUSTIFIER1, -1, get_cursor_color(var.value),
                                "tBlack");
    else
      S9xSetControllerCrosshair(X_JUSTIFIER1, -1, get_cursor_color(var.value),
                                "Black");
  ...
  if (environ_cb(RETRO_ENVIRONMENT_GET_VARIABLE, &var)) {
    ...
    if (color[0] == 't')
      S9xSetControllerCrosshair(X_JUSTIFIER2, -1, get_cursor_color(var.value),
                                "tBlack");
    else
      S9xSetControllerCrosshair(X_JUSTIFIER2, -1, get_cursor_color(var.value),
                                "Black");
  ...
  if (environ_cb(RETRO_ENVIRONMENT_GET_VARIABLE, &var)) {
    ...
    if (color[0] == 't')
      S9xSetControllerCrosshair(X_MACSRIFLE, -1, get_cursor_color(var.value),
                                "tBlack");
    else
      S9xSetControllerCrosshair(X_MACSRIFLE, -1, get_cursor_color(var.value),
                                "Black");
  ...
  if (environ_cb(RETRO_ENVIRONMENT_GET_VARIABLE, &var))
    Settings.BlockInvalidVRAMAccessMaster =
        !strcmp(var.value, "disabled") ? false : true;
  else
  ...
  if (!libretro_supports_option_categories &&
    ...
    if (show_lightgun_settings != show_lightgun_settings_prev) {
      ...
      char lightgun_keys[10][64] = {
          "snes9x_lightgun_mode",        "snes9x_superscope_reverse_buttons",
          "snes9x_superscope_crosshair", "snes9x_superscope_color",
          "snes9x_justifier1_crosshair", "snes9x_justifier1_color",
          "snes9x_justifier2_crosshair", "snes9x_justifier2_color",
          "snes9x_rifle_crosshair",      "snes9x_rifle_color"};
  ...
  if (!libretro_supports_option_categories &&
    ...
    if (show_advanced_av_settings != show_advanced_av_settings_prev) {
      ...
      char av_keys[15][32] = {
          ...
          "snes9x_layer_4",    "snes9x_layer_5",   "snes9x_gfx_clip",
          "snes9x_gfx_transp", "snes9x_sndchan_1", "snes9x_sndchan_2",
          "snes9x_sndchan_3",  "snes9x_sndchan_4", "snes9x_sndchan_5",
          "snes9x_sndchan_6",  "snes9x_sndchan_7", "snes9x_sndchan_8"};
...
float get_aspect_ratio(unsigned width, unsigned height) {
  if (aspect_ratio_mode == ASPECT_RATIO_4_3) {
    ...
    return (4.0f * (MAX_SNES_HEIGHT - height)) /
           (3.0f * (MAX_SNES_WIDTH - width));
  } else if (aspect_ratio_mode == ASPECT_RATIO_1_1) {
    return (float)width / (float)height;
  }

  // OV2: not sure if these really make sense - NTSC is similar to 4:3, PAL
  // looks weird
  double sample_frequency_ntsc = 135000000.0f / 11.0f;
  double sample_frequency_pal = 14750000.0;

  double sample_freq = retro_get_region() == RETRO_REGION_NTSC
                           ? sample_frequency_ntsc
                           : sample_frequency_pal;
    ...
    case RETRO_DEVICE_LIGHTGUN_JUSTIFIER_2:
      if (port == 2) {
        ...
        if (log_cb)
          log_cb(
              RETRO_LOG_ERROR,
              "Invalid Justifier (2P) assignment to port %d, must be port 2.\n",
              port);
...
void retro_cheat_reset() { S9xDeleteCheats(); }
...
static int is_bsx(uint8 *p) {
  if ((p[26] == 0x33 || p[26] == 0xFF) && (!p[21] || (p[21] & 131) == 128) &&
...
static bool8 is_SufamiTurbo_Cart(const uint8 *data, uint32 size) {
  if (size >= 0x80000 && size <= 0x100000 &&
      strncmp((char *)data, "BANDAI SFC-ADX", 14) == 0 &&
  ...
  if (game->data == NULL && game->size == 0 && game->path != NULL)
    ...
    if (is_SufamiTurbo_Cart((uint8 *)game->data, game->size)) {
      if ((rom_loaded = LoadBIOS(biosrom, "STBIOS.bin", 0x40000)))
        rom_loaded = Memory.LoadMultiCartMem(
            (const uint8_t *)game->data, game->size, 0, 0, biosrom, 0x40000);
    }

    else if ((is_bsx((uint8 *)game->data + 0x7fc0) == 1) |
             (is_bsx((uint8 *)game->data + 0xffc0) == 1)) {
      if ((rom_loaded = LoadBIOS(biosrom, "BS-X.bin", 0x100000)))
        rom_loaded = Memory.LoadMultiCartMem(
            biosrom, 0x100000, (const uint8_t *)game->data, game->size, 0, 0);
    }
...
static void remove_header(uint8_t *&romptr, size_t &romsize,
                          bool multicart_sufami) {
  if (romptr == 0 || romsize == 0)
    return;

  uint32 calc_size = (romsize / 0x2000) * 0x2000;
  if ((romsize - calc_size == 512 && !Settings.ForceNoHeader) ||
      Settings.ForceHeader) {
    romptr += 512;
    romsize -= 512;

    if (log_cb)
      log_cb(RETRO_LOG_INFO, "ROM header removed\n");
  }
  ...
  if (multicart_sufami && (romptr + romsize) >= (romptr + 0x100000)) {
    if (strncmp((const char *)(romptr + 0x100000), "BANDAI SFC-ADX", 14) == 0 &&
      ...
      if (log_cb)
        log_cb(RETRO_LOG_INFO, "Sufami Turbo Multi-ROM bios removed\n");
    }
  }
}
    ...
    if (num_info == 2) {
      if (is_SufamiTurbo_Cart((const uint8_t *)romptr[0], romsize[0])) {
        ...
        if (LoadBIOS(biosptr, "STBIOS.bin", 0x40000)) {
          ...
          rom_loaded = Memory.LoadMultiCartMem(
              (const uint8_t *)romptr[0], romsize[0],
        ...
        rom_loaded = Memory.LoadMultiCartMem(
            (const uint8_t *)romptr[0], romsize[0], (const uint8_t *)romptr[1],
            romsize[1], NULL, 0);
      }
    }
...
static void check_system_specs(void) {
  /* TODO - might have to variably set performance level based on
   * SuperFX/SA-1/etc */
  unsigned level = 12;
...
void retro_init(void) {
  ...
  ntsc_screen_buffer =
      (uint16 *)calloc(1, MAX_SNES_WIDTH_NTSC * 2 * (MAX_SNES_HEIGHT + 16));
...
static void map_buttons() {
  ...
  S9xMapPointer(
      (BTN_POINTER),
      S9xGetCommandT("Pointer Mouse1+Superscope+Justifier1+MacsRifle"), false);
  ...
  MAP_BUTTON(MAKE_BUTTON(PAD_2, BTN_SELECT),
             "{Joypad2 Select,Mouse2 L,Superscope Fire,Justifier1 "
             "Trigger,MacsRifle Trigger}");
  MAP_BUTTON(MAKE_BUTTON(PAD_2, BTN_START),
             "{Joypad2 Start,Mouse2 R,Superscope Cursor,Justifier1 Start}");
  MAP_BUTTON(MAKE_BUTTON(PAD_2, BTN_UP),
             "{Joypad2 Up,Superscope ToggleTurbo,Justifier1 AimOffscreen}");
  MAP_BUTTON(MAKE_BUTTON(PAD_2, BTN_DOWN), "{Joypad2 Down,Superscope Pause}");
  MAP_BUTTON(MAKE_BUTTON(PAD_2, BTN_LEFT),
             "{Joypad2 Left,Superscope AimOffscreen}");
  ...
  MAP_BUTTON(MAKE_BUTTON(PAD_3, BTN_SELECT),
             "{Joypad3 Select,Justifier2 Trigger}");
  MAP_BUTTON(MAKE_BUTTON(PAD_3, BTN_START), "{Joypad3 Start,Justifier2 Start}");
  MAP_BUTTON(MAKE_BUTTON(PAD_3, BTN_UP),
             "{Joypad3 Up,Justifier2 AimOffscreen}");
...
static void input_report_gun_position(unsigned port, int s9xinput) {
  ...
  x = input_state_cb(port, RETRO_DEVICE_LIGHTGUN, 0,
                     RETRO_DEVICE_ID_LIGHTGUN_SCREEN_X);
  y = input_state_cb(port, RETRO_DEVICE_LIGHTGUN, 0,
...
static void input_handle_pointer_lightgun(unsigned port, unsigned gun_device,
                                          int s9xinput) {
  int x, y;
  ...
  // Touch sensitivity: Keep the gun position held for a fixed number of cycles
  // after touch is released because a very light touch can result in a misfire
  if (pointer_cycles_after_released > 0 &&
      pointer_cycles_after_released < POINTER_PRESSED_CYCLES) {
  ...
  if (input_state_cb(port, RETRO_DEVICE_POINTER, 0,
    ...
    switch (gun_device) {
    case RETRO_DEVICE_LIGHTGUN_SUPER_SCOPE:
      S9xReportButton(MAKE_BUTTON(PAD_2, setting_superscope_reverse_buttons
                                             ? SUPER_SCOPE_CURSOR
                                             : SUPER_SCOPE_TRIGGER),
                      false);
      break;
  ...
  case RETRO_DEVICE_LIGHTGUN_SUPER_SCOPE: {
    ...
    if (input_state_cb(port, RETRO_DEVICE_POINTER, 0,
                       RETRO_DEVICE_ID_POINTER_PRESSED)) {
      int touch_count = input_state_cb(port, RETRO_DEVICE_POINTER, 0,
  ...
  case RETRO_DEVICE_LIGHTGUN_JUSTIFIER: {
    ...
    S9xReportButton(MAKE_BUTTON(PAD_2, JUSTIFIER_TRIGGER),
                    trigger_pressed || offscreen);
  ...
  case RETRO_DEVICE_LIGHTGUN_MACS_RIFLE: {
    int pressed = input_state_cb(port, RETRO_DEVICE_POINTER, 0,
                                 RETRO_DEVICE_ID_POINTER_PRESSED);
  ...
  for (int port = 0; port <= 1; port++) {
    ...
    case RETRO_DEVICE_JOYPAD:
      if (libretro_supports_bitmasks)
        joy_bits = input_state_cb(port * offset, RETRO_DEVICE_JOYPAD, 0,
                                  RETRO_DEVICE_ID_JOYPAD_MASK);
      else {
        joy_bits = 0;
        for (int i = 0; i < (RETRO_DEVICE_ID_JOYPAD_R3 + 1); i++)
          joy_bits |= input_state_cb(port * offset, RETRO_DEVICE_JOYPAD, 0, i)
                          ? (1 << i)
                          : 0;
      }
    ...
    case RETRO_DEVICE_JOYPAD_MULTITAP:
      for (int j = 0; j < 4; j++) {
        if (libretro_supports_bitmasks)
          joy_bits = input_state_cb(port * offset + j, RETRO_DEVICE_JOYPAD, 0,
                                    RETRO_DEVICE_ID_JOYPAD_MASK);
        else {
          joy_bits = 0;
          for (int i = 0; i < (RETRO_DEVICE_ID_JOYPAD_R3 + 1); i++)
            joy_bits |=
                input_state_cb(port * offset + j, RETRO_DEVICE_JOYPAD, 0, i)
                    ? (1 << i)
                    : 0;
        }

        for (int i = BTN_FIRST; i <= BTN_LAST; i++)
          S9xReportButton(MAKE_BUTTON(port * offset + j + 1, i),
                          joy_bits & (1 << i));
      }
      break;
    ...
    case RETRO_DEVICE_LIGHTGUN_SUPER_SCOPE:
      ...
      if (setting_gun_input == SETTING_GUN_INPUT_POINTER) {
        input_handle_pointer_lightgun(port, RETRO_DEVICE_LIGHTGUN_SUPER_SCOPE,
                                      BTN_POINTER);
      } else {
        // Lightgun is default
        ...
        for (int i = 0; i < scope_button_count; i++) {
          ...
          bool btn =
              input_state_cb(port, RETRO_DEVICE_LIGHTGUN, 0, id) ? true : false;

          /* RETRO_DEVICE_ID_LIGHTGUN_TURBO special case - core needs a
           * rising-edge trigger */
    ...
    case RETRO_DEVICE_LIGHTGUN_JUSTIFIER:
      ...
      if (setting_gun_input == SETTING_GUN_INPUT_POINTER) {
        input_handle_pointer_lightgun(port, RETRO_DEVICE_LIGHTGUN_JUSTIFIER,
                                      BTN_POINTER);
      } else {
        // Lightgun is default
        input_report_gun_position(port, BTN_POINTER);

        {
          ...
          int btn_offscreen_shot = input_state_cb(
              port, RETRO_DEVICE_LIGHTGUN, 0, RETRO_DEVICE_ID_LIGHTGUN_RELOAD);

          /* Trigger ? */
          int btn_trigger = input_state_cb(port, RETRO_DEVICE_LIGHTGUN, 0,
                                           RETRO_DEVICE_ID_LIGHTGUN_TRIGGER);
          S9xReportButton(MAKE_BUTTON(PAD_2, JUSTIFIER_TRIGGER),
                          btn_trigger || btn_offscreen_shot);

          /* Start Button ? */
          int btn_start = input_state_cb(port, RETRO_DEVICE_LIGHTGUN, 0,
                                         RETRO_DEVICE_ID_LIGHTGUN_START);
          S9xReportButton(MAKE_BUTTON(PAD_2, JUSTIFIER_START),
                          btn_start ? 1 : 0);
          ...
          S9xReportButton(MAKE_BUTTON(PAD_2, JUSTIFIER_OFFSCREEN),
                          btn_offscreen || btn_offscreen_shot);
        }
        ...
        if (snes_devices[port + 1] == RETRO_DEVICE_LIGHTGUN_JUSTIFIER_2) {
          ...
          /* Special Reload Button */
          int btn_offscreen_shot =
              input_state_cb(second, RETRO_DEVICE_LIGHTGUN, 0,
                             RETRO_DEVICE_ID_LIGHTGUN_RELOAD);

          /* Trigger ? */
          int btn_trigger = input_state_cb(second, RETRO_DEVICE_LIGHTGUN, 0,
                                           RETRO_DEVICE_ID_LIGHTGUN_TRIGGER);
          S9xReportButton(MAKE_BUTTON(PAD_3, JUSTIFIER_TRIGGER),
                          btn_trigger || btn_offscreen_shot);

          /* Start Button ? */
          int btn_start = input_state_cb(second, RETRO_DEVICE_LIGHTGUN, 0,
                                         RETRO_DEVICE_ID_LIGHTGUN_START);
          S9xReportButton(MAKE_BUTTON(PAD_3, JUSTIFIER_START),
                          btn_start ? 1 : 0);
          ...
          int btn_offscreen =
              input_state_cb(second, RETRO_DEVICE_LIGHTGUN, 0,
                             RETRO_DEVICE_ID_LIGHTGUN_IS_OFFSCREEN);
          S9xReportButton(MAKE_BUTTON(PAD_3, JUSTIFIER_OFFSCREEN),
                          btn_offscreen || btn_offscreen_shot);
        }
      }
      break;
    ...
    case RETRO_DEVICE_LIGHTGUN_MACS_RIFLE:
      ...
      if (setting_gun_input == SETTING_GUN_INPUT_POINTER) {
        input_handle_pointer_lightgun(port, RETRO_DEVICE_LIGHTGUN_MACS_RIFLE,
                                      BTN_POINTER);
      } else {
        input_report_gun_position(port, BTN_POINTER);
...
size_t retro_get_memory_size(unsigned type) {
  ...
  case RETRO_MEMORY_SAVE_RAM:
    ...
    break;
  case RETRO_MEMORY_SNES_SUFAMI_TURBO_B_RAM:
    size = (unsigned)(Multi.cartType == 4 && Multi.sramSizeB
                          ? (1 << (Multi.sramSizeB + 3)) * 128
                          : 0);
    break;
  case RETRO_MEMORY_RTC:
...
size_t retro_serialize_size() { return rom_loaded ? S9xFreezeSize() : 0; }
...
bool8 S9xDeinitUpdate(int width, int height) {
  ...
  if (blargg_filter) {
    ...
    if (width == 512)
      snes_ntsc_blit_hires(snes_ntsc, GFX.Screen, GFX.Pitch / 2, burst_phase,
                           width, height, snes_ntsc_buffer,
                           MAX_SNES_WIDTH_NTSC * 2);
    else
      snes_ntsc_blit(snes_ntsc, GFX.Screen, GFX.Pitch / 2, burst_phase, width,
                     height, snes_ntsc_buffer, MAX_SNES_WIDTH_NTSC * 2);

    video_cb(snes_ntsc_buffer + ((int)(MAX_SNES_WIDTH_NTSC)*overscan_offset),
             SNES_NTSC_OUT_WIDTH(256), height, MAX_SNES_WIDTH_NTSC * 2);
  } else if (width == MAX_SNES_WIDTH && hires_blend) {
#define AVERAGE_565(el0, el1)                                                  \
  (((el0) & (el1)) + ((((el0) ^ (el1)) & 0xF7DE) >> 1))
    ...
    video_cb(GFX.Screen + ((int)(GFX.Pitch >> 1) * overscan_offset), width,
             height, GFX.Pitch);
  } else {
    video_cb(GFX.Screen + ((int)(GFX.Pitch >> 1) * overscan_offset), width,
             height, GFX.Pitch);
